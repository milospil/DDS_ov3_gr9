...
if categorical_cols:
    ...
    if "age_group" in categorical_cols:
        ...
        # Get unique categories to see what we're working with
        unique_categories = sorted(X["age_group"].unique())
        ...
        # Drop original age_group, keep encoded version
        X = X.drop("age_group", axis=1)

        # Rename for clarity
        X = X.rename(columns={"age_group_encoded": "age_group"})

        print(f"  ✓ age_group now encoded as integers (0-{len(le.classes_)-1})")
else:
...
plt.title(
    "Feature Correlation Matrix (Before Selection)", fontsize=16, fontweight="bold"
)
...
anova_results = pd.DataFrame(
    {"feature": X_no_corr.columns, "f_score": f_scores, "p_value": p_values}
...
mi_results = pd.DataFrame(
    {"feature": X_no_corr.columns, "mi_score": mi_scores}
...
plt.tight_layout()
plt.savefig(
    "outputs/figures/mutual_information_scores.png", dpi=300, bbox_inches="tight"
)
plt.close()
...
rf_importance = pd.DataFrame(
    {"feature": X_no_corr.columns, "importance": rf.feature_importances_}
...
plt.tight_layout()
plt.savefig(
    "outputs/figures/rf_feature_importance_selection.png", dpi=300, bbox_inches="tight"
)
plt.close()
...
# Get selected features - Convert boolean to 0/1 for CSV
rfe_selected = X_no_corr.columns[rfe.support_].tolist()
rfe_ranking = pd.DataFrame(
    {
        "feature": X_no_corr.columns,
        "ranking": rfe.ranking_,
        "selected": rfe.support_.astype(int),  # Convert True/False to 1/0
    }
).sort_values("ranking")
...
not_selected = rfe_ranking[rfe_ranking["selected"] == 0]  # Filter by 0
...
medical_features_available = [
    f for f in medical_literature_features if f in X_no_corr.columns
]
...
for feature in all_features:
    ...
    selection_scores[feature] = {"score": score, "methods": methods_selected}
...
# Check if features exist before accessing
if (
    "sex" in anova_results["feature"].values
    and "BMI" in anova_results["feature"].values
):
    ...
    print(f"  sex F-score: {sex_fscore:,.0f}")
    print(f"  BMI F-score: {bmi_fscore:,.0f}")

    if "height" in anova_results["feature"].values:
        height_fscore = anova_results[anova_results["feature"] == "height"][
            "f_score"
        ].values[0]
        print(f"  height F-score: {height_fscore:,.0f}")

    if "weight" in anova_results["feature"].values:
        weight_fscore = anova_results[anova_results["feature"] == "weight"][
            "f_score"
        ].values[0]
        print(f"  weight F-score: {weight_fscore:,.0f}")

    print("\nRationale for removal:")
    print(
        "  1. Males are significantly taller and heavier than females (biological sex difference)"
    )
    print(
        "  2. Males have substantially higher smoking rates (behavioral sex difference)"
    )
    print(
        "  3. Height and weight serve as proxy variables for sex rather than independent predictors"
    )
    print("  4. Sex dominates all methods (2-3x stronger than height/weight)")
    print(
        "  5. BMI captures the meaningful body composition relationship normalized for height"
    )
    ...
    print("  Confounding analysis skipped (required features not present)")
...
colors = [
    (
        "red"
        if score < 3
        else "orange" if score == 3 else "yellowgreen" if score == 4 else "darkgreen"
    )
    for score in selection_plot_df["selection_score"]
]

bars = ax.barh(
    range(len(selection_plot_df)), selection_plot_df["selection_score"], color=colors
)
...
plt.tight_layout()
plt.savefig(
    "outputs/figures/feature_selection_consensus.png", dpi=300, bbox_inches="tight"
)
plt.close()
