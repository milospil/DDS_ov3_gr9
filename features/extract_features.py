...
if categorical_cols:
    ...
    if 'age_group' in categorical_cols:
        ...
        # CRITICAL: Remove continuous 'age' to avoid collinearity with age_group
        if 'age' in X.columns:
            print("\n⚠️  Removing continuous 'age' to avoid redundancy with age_group")
            X = X.drop('age', axis=1)
            print("  Rationale: Age groups are derived from age; keeping both creates collinearity")
else:
...
# Features known from medical literature (removed 'age' since we dropped it)
...
if "sex" in anova_results["feature"].values and "BMI" in anova_results["feature"].values:
...
colors = [
    "red" if score < 3 else "orange" if score == 3 else "yellowgreen" if score == 4 else "darkgreen"
...
print(f"Reduction: {(1 - len(final_selected_features)/df.drop(['SMK_stat_type_cd', 'drink'], axis=1).shape[1])*100:.1f}%")
print(f"\nFiles saved:")
